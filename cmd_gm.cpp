...
struct FuncPurge
{
	...
	void operator () (LPENTITY ent)
	{
		...
		if (!m_bAll && iDist >= 1000)	// 10aEL?RE상z? R獵?것들Rs purge ?지 lE는?.
			return;

#ifdef __FAKE_PLAYER__
		if (pkChr->IsFakePlayer())
			return;
#endif
...
#ifdef ENABLE_NEW_PET_SYSTEM
		if (pkChr->IsNPC() && !pkChr->IsPet() && !pkChr->IsNewPet() && !pkChr->IsBuffNPC() && pkChr->GetRider() == NULL)
#else
...
ACMD(do_state)
{
	...
	ch->ChatPacket(CHAT_TYPE_INFO, "   TREE:%3d%% MONSTER:%3d%% METIN:%3d%%",
#else
...
// Invalid syntax
	if (!*arg1 || !*arg2 || !*arg3)
	{
			ch->ChatPacket(CHAT_TYPE_INFO, "Falsche Eingabe, Benutzung: <Spieler Name> <Zeit in Stunden> <Grund> Benutzt _ und nicht Leertaste!");
		return;
	}
		szName      = arg1;
		iDuration   = atoi(arg2);
	...
	if (iDuration <= 0)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "Dauer des Banns kann nicht 0 oder Minus sein.");
		return;
	}
	...
	if (!tch)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "%s spielt gerade nicht.", szName);
		return;
	}
	 
	if (!tch->GetDesc())
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "%s hat keine Beschreibung.", szName);
		return;
	}
	 
	if (tch == ch)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "Warum willst du dich selbst bannen?!");
		return;
	}
	 
	if (tch->GetGMLevel() > GM_PLAYER)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "Du darfst keine Teamler bannen!");
		return;
	}
	...
	sys_log(0, "%s[%d] gebannt %s f? %i Stunden, Grund daf?: %s", ch->GetName(), ch->GetPlayerID(), szName, iDuration, szReason);
	 
	ch->ChatPacket(CHAT_TYPE_INFO, "%s wurde gebannt f? %i Stunden, Grund daf?: %s", szName, iDuration, szReason);
}
	...
	case JOB_SHAMAN:
		{
			...
			{
				item->ClearAttribute();
				item->SetForceAttribute( 0, APPLY_NORMAL_HIT_DAMAGE_BONUS, 62);
				item->SetForceAttribute( 1, APPLY_SKILL_DAMAGE_BONUS, -29);
				item->SetForceAttribute( 2, APPLY_PENETRATE_PCT, 10);
				item->SetForceAttribute( 3, APPLY_ATTBONUS_DEVIL, 20);
				item->SetForceAttribute( 4, APPLY_STR, 12);
			}
...
ACMD(do_pet_next_level)
{
	if (ch->GetNewPetSystem()->IsActivePet())
		ch->GetNewPetSystem()->SetNextLevel();
}
...
ACMD(do_pet_give_exp)
{
	...
	if (!*arg1 || !*arg2)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "Syntax: pet_give_exp <amount> <type exp 0 / item-exp 1>");
		return;
	}

	int	experience = 0;
	int	type = 0;
	str_to_number(experience, arg1);
	str_to_number(type, arg2);
	
	if (ch->GetNewPetSystem()->IsActivePet())
		ch->GetNewPetSystem()->SetExp(experience, type);
}
#endif
...
ACMD(do_daily_gift_event)
{
	...
	const char* c_szArgBuff;
	c_szArgBuff = two_arguments(argument, szArg1, sizeof(szArg1), szArg2, sizeof(szArg2));
	one_argument(c_szArgBuff, szArg3, sizeof(szArg3));

	BYTE bCommand = 1;
	BYTE bSpecificWeek = 0;
	BYTE bOrderWeeks = 0;
	BYTE bRenew = 0;

	if (!*szArg1)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "Usage: daily_gift_event <command> <specific_week> <renew>");
		ch->ChatPacket(CHAT_TYPE_INFO, "<command>");
		ch->ChatPacket(CHAT_TYPE_INFO, "    0 = Disable");
		ch->ChatPacket(CHAT_TYPE_INFO, "    1 = Enable");
		ch->ChatPacket(CHAT_TYPE_INFO, "<specific_week>");
		ch->ChatPacket(CHAT_TYPE_INFO, "    ID = ( 0 = Random | # Week table )");
		ch->ChatPacket(CHAT_TYPE_INFO, "    STRING = ( auto = Start week from order )");
		ch->ChatPacket(CHAT_TYPE_INFO, "<renew>");
		ch->ChatPacket(CHAT_TYPE_INFO, "    0 = The event will stop after a week");
		ch->ChatPacket(CHAT_TYPE_INFO, "    1 = The event will continue after a week");
		ch->ChatPacket(CHAT_TYPE_INFO, "        with the same arguments.");
		ch->ChatPacket(CHAT_TYPE_INFO, "<renew>");
		ch->ChatPacket(CHAT_TYPE_INFO, "    0 = The event will stop after a week");
		ch->ChatPacket(CHAT_TYPE_INFO, "    1 = The event will continue after a week");
		ch->ChatPacket(CHAT_TYPE_INFO, "        with the same arguments.");
		ch->ChatPacket(CHAT_TYPE_INFO, "Example:");
		ch->ChatPacket(CHAT_TYPE_INFO, "Command: /daily_gift_event 1 auto 1");

		if (ch->CheckDailyGiftStatus())
		{
			time_t tStartTime = ch->GetDailyGiftStartTime();
			time_t tRenewTime = ch->GetDailyGiftRenewTime();

			ch->ChatPacket(CHAT_TYPE_INFO, "");
			ch->ChatPacket(CHAT_TYPE_INFO, "     Start Date: %s", ctime(&tStartTime));
			ch->ChatPacket(CHAT_TYPE_INFO, "     Renew Date: %s", ctime(&tRenewTime));
		}
	...
	if (isnhdigit(*szArg1))
		str_to_number(bCommand, szArg1);

	if (*szArg2 && isnhdigit(*szArg2))
		str_to_number(bSpecificWeek, szArg2);
	else if (*szArg2 && !strcmp(szArg2, "auto"))
		bOrderWeeks = 1;

	if (*szArg3 && isnhdigit(*szArg3))
		str_to_number(bRenew, szArg3);

	if (bCommand == 1)
	{
		if (!ch->CheckDailyGiftStatus())
		{
			DWORD dwRenewTime = time(0) + 60 * 60 * 24 * DAILY_GIFT_WEEK_DAYS + 1;
			BYTE bWeek = 1;
			if (!bOrderWeeks)
				bWeek = bSpecificWeek > 0 ? bSpecificWeek : number(1, ITEM_MANAGER::instance().GetAllDailyGiftGroupSize());

			quest::CQuestManager::instance().RequestSetEventFlag("daily_gift_event", 1);
			quest::CQuestManager::instance().RequestSetEventFlag("daily_gift_week", bWeek);
			quest::CQuestManager::instance().RequestSetEventFlag("daily_gift_start_time", time(0));
			quest::CQuestManager::instance().RequestSetEventFlag("daily_gift_renew_time", dwRenewTime);
			quest::CQuestManager::instance().RequestSetEventFlag("daily_gift_renew", bRenew);
			quest::CQuestManager::instance().RequestSetEventFlag("daily_gift_order", bOrderWeeks);

			SendNotice(LC_TEXT("Daily gifts are now available."));
		}
		else
		{
			ch->ChatPacket(CHAT_TYPE_INFO, "This event is already active.");
		}
	}
	else
	{
		...
		ch->ChatPacket(CHAT_TYPE_INFO, "You disabled the daily gift event.");
	}
}
#endif
...
ACMD(do_banpanel)
{
	...
	if(arg1[0]=='u')
	{
		...
		std::unique_ptr<SQLMsg> pMsg(DBManager::instance().DirectQuery("SELECT account_id FROM player.player WHERE LOWER(name) like LOWER('%s') LIMIT 1", szEscapeName));
	...
	else if(arg1[0]=='b')
	{
		...
		if (!tch)
		{
			ch->ChatPacket(CHAT_TYPE_INFO, "%s: no such player.", szEscapeName);
			return;
		}
	...
	else if(arg1[0]=='d')
	{
		...
		char szEscapeHwid[HWID_MAX_LEN * 2 + 1];
		DBManager::instance().EscapeString(szEscapeHwid, sizeof(szEscapeHwid), row2[0], strlen(row2[0]));
		DBManager::instance().DirectQuery("DELETE FROM account.hwid_ban WHERE hwid = '%s'", szEscapeHwid);
		ch->ChatPacket(CHAT_TYPE_INFO,"%s HWID unbanned (%s)", szEscapeName, szEscapeHwid);
		return;	
	}
	else if(arg1[0]=='t')
	{
		...
		char szEscapeHwid[HWID_MAX_LEN * 2 + 1];
		DBManager::instance().EscapeString(szEscapeHwid, sizeof(szEscapeHwid), row2[0], strlen(row2[0]));
		DBManager::instance().DirectQuery("UPDATE account.account set status = 'BLOCK' WHERE hwid = '%s'", szEscapeHwid);
		DBManager::instance().DirectQuery("INSERT INTO account.hwid_ban (hwid, player, gm) VALUES ('%s', '%s', '%s')", szEscapeHwid, szEscapeName, ch->GetName());
		ch->ChatPacket(CHAT_TYPE_INFO,"%s HWID and all associated accounts banned (%s)", szEscapeName, szEscapeHwid);
		LPDESC d = DESC_MANAGER::instance().FindByCharacterName(szEscapeName);
		LPCHARACTER	tch = d ? d->GetCharacter() : NULL;
		if (!tch)
		{

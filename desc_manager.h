...
class DESC_MANAGER : public singleton<DESC_MANAGER>
{
	...
	public:
		...
		void IncreaseSusConnections(std::string host)
		{
			auto entry = m_map_SusConnections.find(host);
			if (entry == m_map_SusConnections.end())
			{
				m_map_SusConnections.insert(std::make_pair(host, 1));
			}
			else
			{
				entry->second++;
		...
		bool IsSusConnection(std::string host)
		{
			auto entry = m_map_SusConnections.find(host);
			if (entry == m_map_SusConnections.end())
				return false;
		...
		void FlushSusConnections()
		{
			m_map_SusConnections.clear();
		}
#ifdef __FreeBSD__
	...
	private:
		...
		std::map<std::string, uint8_t>	m_map_SusConnections;

...
#ifdef ENABLE_NEW_PET_SYSTEM
DWORD AdjustPetExpByLevel(int lvl, const DWORD exp)
{
	if (PLAYER_EXP_TABLE_MAX < lvl)
	{
		double ret = 0.95;
...
bool CHARACTER::Attack(LPCHARACTER pkVictim, BYTE bType)
{
	...
	if (IsPC())
	{
		LPITEM item = GetWear(WEAR_WEAPON);
		bool isDagger = item && item->GetSubType() == WEAPON_DAGGER;
		bool isFan = item && item->GetSubType() == WEAPON_FAN;
		bool isTwoHanded = item && item->GetSubType() == WEAPON_TWO_HANDED;
#ifdef ENABLE_WOLFMAN
		bool isClaw = item && item->GetSubType() == WEAPON_CLAW;
		DWORD result = GetCShield()->CheckAttackspeedHack(IsRiding(), isDagger || isFan || isClaw, isTwoHanded, ani_attack_speed(this), GetLimitPoint(POINT_ATT_SPEED), pkVictim->GetVID(), get_dword_time());
#else
		DWORD result = GetCShield()->CheckAttackspeedHack(IsRiding(), isDagger || isFan, isTwoHanded, ani_attack_speed(this), GetLimitPoint(POINT_ATT_SPEED), pkVictim->GetVID(), get_dword_time());
#endif
		if (result == 1)
		{
			LogManager::instance().CShieldLog(GetAID(), GetName(), 10005);
			LPDESC d = GetDesc();
			if (d)
	...
	if (GetMobRank() >= MOB_RANK_BOSS && !IsStone() && GetMobTable().dwGoldMax != 0)
	{
		...
		for (int i = 0; i < iSplitCount; ++i)
		{
			...
			if ((item = ITEM_MANAGER::instance().CreateItem(1, iGold)))
			{
				...
				if (pkAttacker->FindAffect(AFFECT_AUTO_PICKUP))			
				{					
					pos.x = pkAttacker->GetX();
					pos.y = pkAttacker->GetY();				
					pkAttacker->PickupItem(item->GetVID());
				}	
#endif				
			}
		}
		...
		if (iGold != 0)
		{
			...
			for (int i = 0; i < iSplitCount; ++i)
			{
				if (isAutoLoot)
				{
					...
					pkAttacker->UpdateExtBattlePassMissionProgress(YANG_COLLECT, iGold / iSplitCount, GetMapIndex());
#endif
...
void CHARACTER::Reward(bool bItemDrop)
{
	...
	if (pkAttacker->IsPC() && !this->IsPC())
	{
		if (pkAttacker->IsMultiFarmBlocked())
		{
			if (GetMobRank() >= MOB_RANK_BOSS)
			{
				int normalPlayer = 0;
	...
	if (ITEM_MANAGER::instance().CreateDropItem(this, pkAttacker, s_vec_item))
	{
		...
		else if (s_vec_item.size() == 1)
		{
			...
			if (auto pAffect = pkAttacker->FindAffect(AFFECT_AUTO_PICKUP); pAffect) 
			{
				if (auto dstItem = ITEM_MANAGER::instance().Find(pAffect->dwFlag); dstItem)
				{
					if (!pkAttacker->IsFilteredPickupItem(item->GetVnum(), static_cast<uint16_t>(dstItem->GetSocket(1))))
					{
						pos.x = pkAttacker->GetX();
						pos.y = pkAttacker->GetY();
						pkAttacker->PickupItem(item->GetVID());
		...
		else
		{
			...
			if ( 292 or 2997 == GetRaceNum()) //World boss
			{
				while (!pq.empty() && pq.top().first * 100 >= total_dam)
				{
...
void CHARACTER::ItemDropPenalty(LPCHARACTER pkKiller)
{
	...
	if (bDropInventory) // Drop Inventory
	{
		...
		if (!vec_bSlots.empty())
		{
			...
			for (i = 0; i < iQty; ++i)
			{
				...
				SyncQuickslot(QUICKSLOT_TYPE_ITEM, vec_bSlots[i], MAX_QUICKSLOT);
	...
	if (bDropAntiDropUniqueItem)
	{
		...
		if (pkItem && pkItem->GetVnum() == UNIQUE_ITEM_SKIP_ITEM_DROP_PENALTY)
		{
			SyncQuickslot(QUICKSLOT_TYPE_ITEM, WEAR_UNIQUE2, MAX_QUICKSLOT);
			vec_item.push_back(std::make_pair(pkItem->RemoveFromCharacter(), EQUIPMENT));
		}
	}
	...
	if (pkKiller && pkKiller->IsPC())
	{
		...
		{	
			...
			if (betMoneyDead > 0 && betMoneyKiller > 0)
			{
				...
				pkKiller->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("duel_reward_money_win"), betMoneyDead);
			}
...
bool CHARACTER::Damage(LPCHARACTER pAttacker, int dam, EDamageType type) // returns true if dead
{
...
#endif
			&& CMonarch::instance().IsPowerUp(pAttacker->GetEmpire()))
		{
			// 10% 피해 증가
			dam += dam / 10;
		}
...
#endif			
		&& CMonarch::instance().IsDefenceUp(GetEmpire()))
		{
			// 10% 피해 감소
			dam -= dam / 10;
		}
	...
	if (pAttacker)
	{
		...
		if (pAttacker)
		{
			...
			if( pAttacker->IsDito() )
			{
				LPCHARACTER owner = pAttacker->m_pkDito ? pAttacker->m_pkDito->GetOwner() : NULL;
				if (owner)
				{
					int back=dam;
					dam = pAttacker->m_pkDito ? pAttacker->m_pkDito->GetValue(back) : back;
					
					SendDamagePacket(owner, dam, damageFlag);
				}
			}
			else
			{
...
static void GiveExp(LPCHARACTER from, LPCHARACTER to, int iExp)
{
...
#endif
#ifdef ENABLE_NEW_PET_SYSTEM
	int iPetExp = CALCULATE_VALUE_LVDELTA(to->GetNewPetSystem()->GetLevel(), from->GetLevel(), iExp);
	
	if (to->IsEquipUniqueItem(NEW_PET_EXP_RING_ITEM_1) or to->IsEquipUniqueItem(NEW_PET_EXP_RING_ITEM_2) or to->IsEquipUniqueItem(NEW_PET_EXP_RING_ITEM_3))
		iPetExp += iPetExp * 50 / 100;
#endif
	...
	if (to->GetNewPetSystem()) {
		if (to->GetNewPetSystem()->IsActivePet() && to->GetNewPetSystem()->GetLevelStep() < 4) {
			to->GetNewPetSystem()->SetExp(iPetExp, 0);

			for (CNewPetSystem::TNewPetActorMap::iterator iter = to->GetNewPetSystem()->m_petActorMap.begin();iter != to->GetNewPetSystem()->m_petActorMap.end(); ++iter){
				if(CNewPetActor* petActor = iter->second)
...
#ifdef KOECHER_DES_BOGENSCHUETZEN
...
#ifdef ENABLE_CHECK_ATTACKSPEED_HACK
							DWORD result = m_me->GetCShield()->CheckAttackspeedBowHack(ani_attack_speed(m_me), m_me->GetLimitPoint(POINT_ATT_SPEED), get_dword_time());
							if (result == 1)
							{
								LogManager::instance().CShieldLog(m_me->GetAID(), m_me->GetName(), 10005);
...
int CHARACTER::GetRealReborn() const
{
	return m_iRealReborn;
}

void CHARACTER::ShowReborn(bool bShow)
{
	if (bShow)
	{
		if (m_iReborn != m_iRealReborn)
		{
			m_iReborn = m_iRealReborn;
			UpdatePacket();
		}
	}
	else
...
void CHARACTER::UpdateReborn(int iAmount)
{
	bool bShow = false;
	...
	m_iRealReborn = MINMAX(0, m_iRealReborn + iAmount, 3);
...
struct FuncAggregateMonster
{
	...
	void operator()(LPENTITY ent)
	{
		if (ent->IsType(ENTITY_CHARACTER))
		{
			...
			if (ch->GetVictim())
				return;
			if (DISTANCE_APPROX(ch->GetX() - m_ch->GetX(), ch->GetY() - m_ch->GetY()) < 10000) // Tapferkeitsumhang range
				if (ch->CanBeginFight())
					ch->BeginFight(m_ch);
		}
	}

...
void CHARACTER::HorseSummon(bool bSummon, bool bFromFar, DWORD dwVnum, const char* pPetName)
{
	if ( bSummon )
	{
		...
		DWORD vnum = (0 == dwVnum) ? GetMyHorseVnum() : dwVnum;
		...
		else
		{
			...
			if (pkMob)
			{
				std::stringstream ss("");
				ss << GetName() << "'s ";
				ss << pkMob->m_table.szLocaleName;
				m_chHorse->m_stName = ss.str();
			}
			else
			{
...
DWORD CHARACTER::GetMyHorseVnum() const
{
	...
	if (mountItem && mountItem->GetValue(5) != 0)
		return mountItem->GetValue(5);

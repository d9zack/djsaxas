...
EVENTFUNC(dungeon_dead_event)
{
	...
	{
		if(pDungeon->IsDungeonEmpty())
		{
			if(!pDungeon->GetFlag("duration_for_empty"))
			{
				pDungeon->SetFlag("duration_for_empty", 1);
				pDungeon->SetFlag("back_dungeon_time", time(0) + (60 * 10));//10min
			}
			else
			{
				if(pDungeon->GetFlag("back_dungeon_time") - time(0) <= 0)
				{
					pDungeon->SetFlag("can_back_dungeon", 0);// CLOSE
					return PASSES_PER_SEC(0);
				}
			}
		}
		else
		{
			pDungeon->SetFlag("duration_for_empty", 0);
			pDungeon->SetFlag("back_dungeon_time", 0);
...
#ifdef __BACK_DUNGEON__
bool CDungeon::IsDungeonEmpty()
{
	return m_set_pkCharacter.empty();
}
#endif
	...
	struct FKillSectree
	{
		void operator () (LPENTITY ent)
		{
			if (ent->IsType(ENTITY_CHARACTER))
			{
				...
				if (!ch->IsPC() && !ch->IsPet() && !ch->IsNewPet() && !ch->IsSupport() && !ch->IsBuffNPC())
#else
				if (!ch->IsPC() && !ch->IsPet() && !ch->IsSupport())
#endif
					ch->Dead();
			}
		}
	...
	struct FSendDungeonUIValue
	{
		...
		void operator () (LPENTITY ent)
		{
			if (ent->IsType(ENTITY_CHARACTER))
			{
				...
				if (ch->IsPC())
				{ 
					ch->ChatPacket(CHAT_TYPE_COMMAND, "dungeonui %d", iTimeValue);
				}
			}
		}
	};
	...
	struct FPurgeSectree
	{
		void operator () (LPENTITY ent)
		{
			if (ent->IsType(ENTITY_CHARACTER))
			{
				...
				if (!ch->IsPC() && !ch->IsPet() && !ch->IsBuffNPC() && !ch->IsSupport()) {
					M2_DESTROY_CHARACTER(ch);
				}
			}
...
#ifdef ENABLE_DUNGEON_EVENT_TIMER
void CDungeon::SetDungeonUIValue(int value)
{
	LPSECTREE_MAP pkMap = SECTREE_MANAGER::instance().GetMap(m_lMapIndex);
	if (pkMap == NULL) {
		sys_err("CDungeon: SECTREE_MAP not found for #%ld", m_lMapIndex);
	...
	SetFlag("uivalue", time(0) + value);
	FSendDungeonUIValue f;
	f.iTimeValue = value;
	pkMap->for_each(f);
}
int CDungeon::GetDungeonUIValue()
{
	return MAX(0, GetFlag("uivalue") - time(0));
}
#endif
...
struct FChangeBuffParty
{
	FChangeBuffParty(DWORD dwVnum, LPCHARACTER pkAttacker)
		: m_dwVnum(dwVnum), m_pkAttacker(pkAttacker)
		{
		}

	void operator () (LPCHARACTER ch)
	{
		if (ch != NULL && (m_dwVnum == 94 || m_dwVnum == 95 || m_dwVnum == 96 || m_dwVnum == 110 || m_dwVnum == 111))
		{
			BYTE skill = 0;
			BYTE id = 0;
			switch (m_dwVnum)
			{
				case 94:
					skill = ESkillMotionLength::BUFF_BEGIN + 0;
					id = 3;
					break;
				case 95:
					skill = ESkillMotionLength::BUFF_BEGIN + 1;
					id = 4;
					break;
				case 96:
					skill = ESkillMotionLength::BUFF_BEGIN + 2;
					id = 5;
					break;
				case 110:
					skill = ESkillMotionLength::BUFF_BEGIN + 3;
					id = 4;
					break;
				case 111:
					skill = ESkillMotionLength::BUFF_BEGIN + 4;
					id = 5;
					break;
				default:
					break;
			...
			BYTE data[ESkillMotionLength::MAX_SKILL_COUNT + ESkillMotionLength::MAX_BUFF_COUNT];
			memcpy(data, ch->GetSkillMotion(), sizeof(data));

			BYTE dataAttacker[ESkillMotionLength::MAX_SKILL_COUNT + ESkillMotionLength::MAX_BUFF_COUNT];
			memcpy(dataAttacker, m_pkAttacker->GetSkillMotion(), sizeof(dataAttacker));

			data[skill] = dataAttacker[id];
			ch->SetSkillMotion(data);
		}		
	}
...
void CHARACTER::ChangeBuffParty(DWORD dwVnum, LPCHARACTER pkAttacker)
{
	FChangeBuffParty f(dwVnum, pkAttacker);
...
bool CHARACTER::IsLearnableSkill(DWORD dwSkillVnum) const
{
	...
	if (GetSkillGroup() == 0)
		return false;
	if (pkSkill->dwType != 143 || pkSkill->dwType != 144 || pkSkill->dwType != 145 || pkSkill->dwType != 146 || pkSkill->dwType != 147)
		return true;
...
// #ifdef LWT_BUFFI_SYSTEM
	...
	// if (BuffiSkilliMi(pkSkill->dwType))
		// return true;
// #endif
...
bool CHARACTER::LearnGrandMasterSkill(DWORD dwSkillVnum)
{
	...
	this->EffectPacket(SE_TECHSTUFE_UP);
	return true;
}
...
{
	bool bCanUseHorseSkill = CanUseHorseSkill();
...
#ifdef ENABLE_ASLAN_BUFF_NPC_SYSTEM
	if(isBuffNPC == true)
		pkSk->SetPointVar("iq", GetBuffNPCInt());
	else
		pkSk->SetPointVar("iq", GetPoint(POINT_IQ));
#else
	pkSk->SetPointVar("iq", GetPoint(POINT_IQ));
...
bool CHARACTER::UseSkill(DWORD dwVnum, LPCHARACTER pkVictim, bool bUseGrandMaster)
{
	...
	if (JOB_SURA == GetJob())
	{
		auto item = GetWear(WEAR_WEAPON);
		if (!item)
		{
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT("You need a weapon to perform this skill."));
			return false;
		}
	}
	...
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("yemez"));
		return false;
	}
#endif
	...
	if (pkVictim != NULL && (dwVnum == 94 || dwVnum == 95 || dwVnum == 96 || dwVnum == 110 || dwVnum == 111))
	{
		...
		BYTE data[ESkillMotionLength::MAX_SKILL_COUNT + ESkillMotionLength::MAX_BUFF_COUNT];
		memcpy(data, pkVictim->GetSkillMotion(), sizeof(data));

		BYTE dataAttacker[ESkillMotionLength::MAX_SKILL_COUNT + ESkillMotionLength::MAX_BUFF_COUNT];
		memcpy(dataAttacker, this->GetSkillMotion(), sizeof(dataAttacker));

		data[skill] = dataAttacker[id];
		pkVictim->SetSkillMotion(data);
	}
#endif

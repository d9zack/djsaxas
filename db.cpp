...
void DBManager::SendAuthLogin(LPDESC d)
{
	...
	strlcpy(ptod.lang, r.lang, sizeof(ptod.lang));
		...
		case QID_AUTH_LOGIN:
			{
				...
				else
				{
					...
					sys_err("szHWID: %s", szHWID);
					if (strncmp (szHWID, "", 2) != 0)
					{
					...
					sys_err("szHWID_new: %s", szHWID_new);
					if (!row[col])
				   	{
					   	sys_err("error column %d", col);
						M2_DELETE(pinfo);
					...
					strlcpy(szLang, row[col++], sizeof(szLang));
					if (!row[col]) 
					{
						sys_err("error column %d", col); 
						M2_DELETE(pinfo);
					...
					if (!g_iUseLocale)
					{
						...
						str_to_number(aiPremiumTimes[PREMIUM_AUTOHUNT], row[col++]);
					}
					else
					{
						str_to_number(aiPremiumTimes[PREMIUM_EXP], row[col++]);
					...
					else
					{
						if (LC_IsEurope())
						{
							...
							char szHWID_new2[HWID_MAX_LEN * 2 + 1];
						...
						strlcpy(r.lang, szLang, sizeof(r.lang));
		...
		case QID_AUTH_LOGIN_OPENID:
			{
				...
				else
				{
					...
					char szLang[ACCOUNT_LANG_MAX_LEN + 1] = { 0, };
#endif
					char szStatus[ACCOUNT_STATUS_MAX_LEN + 1];
					DWORD dwID = 0;
		...
		case QID_BRAZIL_CREATE_ID :
			{
				...
				else
				{
					...
					ReturnQuery(QID_AUTH_LOGIN, qi->dwIdent, pinfo,
							"SELECT PASSWORD('%s'),password,securitycode,social_id,'%s',id,status,availDt - NOW() > 0,"
							...
							pinfo->passwd, pinfo->hwid, pinfo->login) ;
				}
			}
			break;

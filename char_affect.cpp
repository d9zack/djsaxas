...
bool IsKeepAffect(BYTE bAffectType)
{
	switch (bAffectType)
	{
/*
		case (AFFECT_MOV_SPEED):                   // IDK 
		case (AFFECT_ATT_SPEED):                   // IDK 
		case (AFFECT_STR):                         // IDK 
		case (AFFECT_DEX):                         // IDK 
		case (AFFECT_INT):                         // IDK 
		case (AFFECT_CON):                         // IDK 
		case (AFFECT_CHINA_FIREWORK):              // IDK 
*/
		case (SKILL_JEONGWI):
...
#define IS_NO_CLEAR_ON_DEATH_AFFECT_PVP(type) ((type) == AFFECT_BLOCK_CHAT || ((type) >= 500 && (type) < 600))
#if defined(__NEW_BLEND_AFFECT__) && defined(__EXTENDED_BLEND_AFFECT__)
	#define IS_NO_CLEAR_ON_DEATH_AFFECT(type) ((type) == AFFECT_BLOCK_CHAT || ((type) >= 500 && (type) < 600) || ((type) >= 304 && (type) <= 317))
#else
...
void CHARACTER::ClearAffect(bool bSave, bool isPcKill)
{
...
#endif
				if ( IS_NO_CLEAR_ON_DEATH_AFFECT_PVP(pkAff->dwType) || IS_NO_SAVE_AFFECT(pkAff->dwType) 
#ifdef __AUTO_PICKUP__		
					|| pkAff->dwType == EAffectTypes::AFFECT_AUTO_PICKUP
#endif		
...
#ifdef __AUTO_PICKUP__			
 			|| AFFECT_AUTO_PICKUP == pElements->dwType
#endif 		
		)
		{
...
#ifdef __AUTO_PICKUP__			
 	if (pkAff->dwType == AFFECT_AUTO_PICKUP)
	{
		if (bAdd)
...
#ifdef ENABLE_PREMIUM_SYSTEM
void CHARACTER::SetAffectFlag(DWORD dwAff)
{
	return m_afAffectFlag.Set(dwAff);
}

void CHARACTER::ResetAffectFlag(DWORD dwAff)
{
	return m_afAffectFlag.Reset(dwAff);
}
#endif

#ifdef ENABLE_CLONE_SYSTEM
void CHARACTER::RemoveAffectFlag(DWORD dwAff)
{
	m_afAffectFlag.Reset(dwAff);
}
#endif

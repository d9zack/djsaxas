...
int battle_melee_attack(LPCHARACTER ch, LPCHARACTER victim)
{
	...
	if(ch != NULL && ch->IsPC() && victim != NULL)
	{
		...
		if (false == victim->IsPC() && victim->GetRaceNum() != 6191 && victim->GetRaceNum() != 6091 && victim->GetRaceNum() != 2291)
...
int CalcAttBonus(LPCHARACTER pkAttacker, LPCHARACTER pkVictim, int iAtk)
{
	...
	)
	{
		...
		if (pkAttacker->IsRaceFlag(RACE_FLAG_ATT_ELEC))
			...
			iAtk -= (iAtk * pkVictim->GetPoint(POINT_RESIST_FIRE)) / 100;
		else if (pkAttacker->IsRaceFlag(RACE_FLAG_ICE))
			iAtk -= (iAtk * pkVictim->GetPoint(POINT_RESIST_ICE)) / 100;
		else if (pkAttacker->IsRaceFlag(RACE_FLAG_ATT_WIND))
			iAtk -= (iAtk * pkVictim->GetPoint(POINT_RESIST_WIND)) / 100;
		else if (pkAttacker->IsRaceFlag(RACE_FLAG_ATT_EARTH))
			iAtk -= (iAtk * pkVictim->GetPoint(POINT_RESIST_EARTH)) / 100;
		else if (pkAttacker->IsRaceFlag(RACE_FLAG_ATT_DARK))
			iAtk -= (iAtk * pkVictim->GetPoint(POINT_RESIST_DARK)) / 100;
	}
...
#ifdef ENABLE_CLONE_SYSTEM
	if (pkAttacker->IsDito())
	{
		LPCHARACTER pOwner=pkAttacker->m_pkDito ? pkAttacker->m_pkDito->GetOwner() : NULL;
		if (pOwner)
			pWeapon = pOwner->GetWear(WEAR_WEAPON);
	}
#endif
...
int CalcArrowDamage(LPCHARACTER pkAttacker, LPCHARACTER pkVictim, LPITEM pkBow, LPITEM pkArrow, bool bIgnoreDefense, bool bIgnoreDistance)
#else 
...
#ifdef KOECHER_DES_BOGENSCHUETZEN
	int iDist = 0, iGap = 0, iPercent = 0;
	if (!bIgnoreDistance)
	{
		iDist = (int) (DISTANCE_SQRT(pkAttacker->GetX() - pkVictim->GetX(), pkAttacker->GetY() - pkVictim->GetY()));
		iGap = (iDist / 100) - 5 - pkAttacker->GetPoint(POINT_BOW_DISTANCE);
		iPercent = 100 - (iGap * 5);
...
int battle_hit(LPCHARACTER pkAttacker, LPCHARACTER pkVictim, int & iRetDam)
{
	...
	if(pkAttacker != NULL && pkAttacker->IsPC() && pkVictim != NULL)
	{
		const bool bAttacking = (get_dword_time() - pkAttacker->GetLastAttackTime()) < pkAttacker->IsRiding() ? 800 : 750;
		if (!bAttacking)
			return BATTLE_NONE;

		if (false == pkVictim->IsPC() && pkVictim->GetRaceNum() != 6191 && pkVictim->GetRaceNum() != 6091 && pkVictim->GetRaceNum() != 2291)
...
DWORD GET_ATTACK_SPEED(LPCHARACTER ch)
{
	...
	DWORD default_bonus = SPEEDHACK_LIMIT_BONUS;    // ���θ� ����(�⺻ 80)

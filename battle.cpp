	...
	if (!victim->IsBuilding())
	{
		...
		else
		{
			...
			&& ! victim->IsFakePlayer()
				...
				max = MAX(750, (int) (victim->GetMobAttackRange() * 1.15f));
		}
...
int CalcAttBonus(LPCHARACTER pkAttacker, LPCHARACTER pkVictim, int iAtk)
{
	...
	if (pkVictim->IsNPC())
	{
		...
		else if (pkVictim->IsRaceFlag(RACE_FLAG_SHADOW))
			iAtk += (iAtk * pkAttacker->GetPoint(POINT_ATTBONUS_SHADOW)) / 100;
#endif
...
#endif
{
	...
	if (pkAttacker->IsPC())
	{
		...
		{
			LogManager::instance().CShieldLog(pkAttacker->GetAID(), pkAttacker->GetName(), 10000);
